...
/**
 * 远程工具模式左侧会话树，原样搬自宿主 `src-web/src/components/RemoteTool/SessionTree.tsx`
 * （参考截图二的 FinalShell 会话列表）：分组可折叠嵌套，连接按协议区分图标，双击
 * SSH 打开终端 / RDP 打开远程桌面，右键给更多动作。树形结构是渲染期从
 * `sessionTreeStore` 的扁平列表按 parent_id/group_id 现算出来的。
 */
export const SessionTree: React.FC = () => {
  ...
  // 应用版本来自 Cargo.toml（standalone/tauri.conf.json 缺省读它），运行时用
  // getVersion() 取，不在前端另存一份容易脱节的版本号。
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div className="session-tree-footer" title={`roc_desk-ssh v${appVersion ?? "?"}`}>
        roc_desk-ssh v{appVersion ?? "…"}
      </div>
