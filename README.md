# SSH/SFTP/Agent/RDP 工具

`roc_desk-ssh` 是 roc_desk 多仓库拆分后的独立工具仓库。

## 用途

提供 SSH 终端、SFTP 文件传输、Windows Agent 连接和 RDP 远程桌面能力。

## 依赖

- 公共基础库：`roc_desk-common` 的 `roc_desk_core`，固定使用对应 tag。
- 工具专属依赖：roc_desk-common core；russh/russh-sftp；Windows Agent protocol；Windows RDP 外壳。
...
默认生成 Release 版本：`bin\roc_desk-ssh.exe`。开发构建使用：
...
```markdown
![SSH/SFTP/Agent/RDP 工具主界面](docs/screenshots/main.png)
```
